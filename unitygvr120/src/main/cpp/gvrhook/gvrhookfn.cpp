...
//        __android_log_print(ANDROID_LOG_INFO, "MJDD", "---FPS--- AvgFPS = %06.2f , Frame = [%06.2f , %06.2f] (ms)", currentFPS, minTime, maxTime);

        LOGITAG("mjgvr","---ShowFPS--- AvgFPS = %06.2f , Frame = [%06.2f , %06.2f] (ms), tid=%d",
                currentFPS, minTime, maxTime, gettid());
...
gvr_context * (*old_gvr_create)(int env, int *app_context, int  class_loader) = NULL;
gvr_context * mj_gvr_create(int env, int *app_context, int  class_loader)
{
//    CallStack stack;
//    stack.dump();
//	sys_call_stack();
