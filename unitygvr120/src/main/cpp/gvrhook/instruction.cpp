...
static int relocateInstructionInThumb16( uint32_t pc, uint16_t instruction, uint16_t *trampoline_instructions )
{
    ...
    else if ( type == ADD_THUMB16 )
    {
        ...
        trampoline_instructions[1] = 0x4802 | ( r << 8 );
        trampoline_instructions[2] = ( instruction & 0xFF87 ) | ( r << 3 );
        trampoline_instructions[3] = 0xBC00 | ( 1 << r );
        trampoline_instructions[4] = 0xE002;
    ...
    else if ( type == MOV_THUMB16 || type == ADR_THUMB16 || type == LDR_THUMB16 )
    {
        ...
        trampoline_instructions[0] = 0x4800 | ( r << 8 );
    ...
    else if ( type == CB_THUMB16 )
    {
        ...
        trampoline_instructions[3] = 0xF000;
...
static int relocateInstructionInThumb32( uint32_t pc, uint16_t high_instruction, uint16_t low_instruction, uint16_t *trampoline_instructions )
{
    ...
    else if ( type == TBB_THUMB32 || type == TBH_THUMB32 )
    {
        ...
        trampoline_instructions[1] = 0x4805 | ( r << 8 );
        trampoline_instructions[2] = 0x4600 | ( rm << 3 ) | rx;
        if ( type == TBB_THUMB32 )
        {
            trampoline_instructions[3] = 0xEB00 | r;
            trampoline_instructions[4] = 0x0000 | ( rx << 8 ) | rx;
            trampoline_instructions[5] = 0x7800 | ( rx << 3 ) | rx;
        }
        else if ( type == TBH_THUMB32 )
        {
            trampoline_instructions[3] = 0xEB00 | r;
            trampoline_instructions[4] = 0x0040 | ( rx << 8 ) | rx;
            trampoline_instructions[5] = 0x8800 | ( rx << 3 ) | rx;
        }
        trampoline_instructions[6] = 0xEB00 | r;
        trampoline_instructions[7] = 0x0040 | ( r << 8 ) | rx;
        trampoline_instructions[8] = 0x3001 | ( r << 8 );
        trampoline_instructions[9] = 0xBC00 | ( 1 << rx );
        trampoline_instructions[10] = 0x4700 | ( r << 3 );
...
void relocateInstruction_1( uint32_t target_addr, void *orig_instructions, int length, void *trampoline_instructions, int *orig_boundaries, int *trampoline_boundaries, int *count )
{
    if ( ( target_addr & 1 ) == 1 )
    {
